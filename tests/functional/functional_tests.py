...
def bad_message_test():
    msg = Message(body="i am a bad message",
                  topic="not_log", from_host="localhost")
    out = FunctionalFilterTestFilter.handle(msg)
    eq_(str(out), "[BAD MESSAGE] i am a bad message")

    msg = Message(body="i am an important log!",
                  topic="log_message", from_host="localhost")
    out = FunctionalFilterTestFilter.handle(msg)
    eq_(str(out),
        "[IMPORTANT] i am an important log! [04f6255dd0c703fc555e7605e3e1df73]"
        )

    msg = Message(body="i am a regular log!",
                  topic="log_message", from_host="localhost")
    out = FunctionalFilterTestFilter.handle(msg)
