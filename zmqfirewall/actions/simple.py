...
class FilterTopicAction(base.Action):
    ...
    def __init__(self,
                 topics,
                 on_success=AcceptMessageAction,
                 on_failure=DropMessageAction):
        self.topics = topics
        if isinstance(on_failure, str):
            ...
            raise ValueError('on_failure must be a string or Action, got %r' %
                             on_failure)
        ...
        if isinstance(on_success, str):
            ...
            raise ValueError('on_success must be a string or Action, got %r' %
                             on_success)
